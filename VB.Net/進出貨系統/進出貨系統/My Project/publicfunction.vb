...
Module publicfunction
    ...
    Public Sub LoginUser(ByVal userid As Object, ByVal pass As Object)
        Try
            ...
            With cmd
                .Connection = con
                .CommandText = "SELECT * FROM tblUser WHERE U_UNAME ='" & userid.Text & "' AND U_PASS = '" & pass.Text & "' AND 檢查碼=1"
            End With
            ...
            If dt.Rows.Count > 0 Then
                If dt.Rows(0).Item("U_TYPE") = "管理者" Then
                    ShowForm(LoginForm1, Form1)
                    LoginForm1.Hide()
                    loginlog(userid.Text)
                ElseIf dt.Rows(0).Item("U_TYPE") = "員工" Then
                    With Form1
                        .product.Visible = False
                        .管理ToolStripMenuItem.Visible = False
                    End With
                    With Forminventorylist
                        .insertin.Visible = False
                        .delin.Visible = False
                        .insertout.Visible = False
                        .delsto.Visible = False
                        .btnrfin.Visible = False
                        .edstn.Visible = False
                        .edsto.Visible = False
                    End With
                    ShowForm(LoginForm1, Form1)
                    LoginForm1.Hide()
                    loginlog(userid.Text)
                ElseIf dt.Rows(0).Item("U_TYPE") = "業務部" Then
                    With Form1
                        .管理ToolStripMenuItem.Visible = False
                    End With
                    ShowForm(LoginForm1, Form1)
        ...
        End Try
    End Sub
    Public Sub loginlog(ByVal account As String)
        Try
            'DECLARE A STRING VARIABLE TO HOLD THE PATH AND THE NAME OF THE TEXT FILE 
            ...
            Using stream_writer As New StreamWriter(path_file, True)
                ...
                End If
                'WRITE THE ERROR MESSAGE.
                stream_writer.WriteLine("登入時間及登入者 " & DateTime.Now & " :: " & account)
            End Using
        Catch ex As Exception
            MsgBox(ex.Message & "at logs sub")
        End Try
